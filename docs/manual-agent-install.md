...
Create the credential-vault key outside PostgreSQL before enabling these
actions. The compose deployment expects the file below to exist on the host;
it is intentionally not generated by the application:
...
Back up this key with the database. A database backup without it cannot
decrypt SSH credentials. Never commit the file or put the key in `.env`.
