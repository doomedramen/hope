...
test.describe("agent deployment", () => {
  ...
  test("serves a verified agent release from the web origin", async ({ request }) => {
    ...
    expect(await response.text()).toMatch(/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?\n$/);
  });

  test("manual command installs and checks in from a Linux target", async ({ page, agentTarget }) => {
    await page.goto("/agents");
    await page.getByRole("button", { name: "Enroll agent" }).click();
    const dialog = page.getByRole("dialog");
    await dialog.getByLabel("Agent connection address").fill("http://host.docker.internal:18443");
    const command = await dialog.locator("pre").innerText();
    expect(command).toContain("HOPE_TLS_PIN=");
    try {
      await execFileAsync("docker", ["exec", agentTarget.containerName, "bash", "-lc", command], { timeout: 120_000 });
    } catch (error) {
      const stderr = String((error as { stderr?: string }).stderr ?? "")
        .replace(/[0-9a-f]{64}\.[0-9a-f]{64}/g, "[redacted enrollment code]");
      throw new Error(`the generated manual agent command failed on the disposable target: ${stderr}`);
    }
    const hostname = (await execFileAsync("docker", ["exec", agentTarget.containerName, "hostname"])).stdout.trim();
    await dialog.getByRole("button", { name: "Done" }).click();
    await expect(page.locator("tbody tr").filter({ hasText: hostname })).toContainText("Online", { timeout: 45_000 });
  });
  ...
  test("installs and enrolls a disposable Linux target from infrastructure", async ({
    ...
    await installDialog.getByLabel("SSH host").fill(process.env.E2E_AGENT_SSH_HOST ?? agentTarget.host);
    await installDialog.getByLabel("Port").fill(String(agentTarget.port));
    await installDialog.getByLabel("Agent connection address").fill("http://host.docker.internal:18443");
